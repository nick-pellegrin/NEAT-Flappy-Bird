...
def plot_spikes(spikes, inputs, title):
...
def simulate(genome, config):
    ...
    for i in range(10000):
        t = dt * i
        # net.set_inputs([10*math.sin(i/100)])
        # inputs.append([10*math.sin(i/100)])
        net.set_inputs([100])
        inputs.append([100])
        output = net.advance(dt)
    ...
    print(genome)
    plot_spikes(neuron_data[net.outputs[0]], inputs, "Network Simulation to Sine Wave")
...
def run(config_path):
    ...
    winner = pop.run(pe.evaluate, 1) # 1 generation
...
if __name__ == '__main__':
    ...
    run(os.path.join(local_dir, 'config-XOR.txt'))